...
def seq_f1_with_mask(all_true_labels, all_pred_labels, all_label_mask, label_vocab):
    """
    For Chinese, since label is given to each character, do not exist subtoken,
    ...
    for i in range(sample_num):
        ...
        for j in range(0, real_seq_length):
            if all_label_mask[i][j] == 1:
                ...
                tmp_true.append(label_vocab.convert_id_to_item(all_true_labels[i][j]))
                tmp_pred.append(label_vocab.convert_id_to_item(all_pred_labels[i][j]))
